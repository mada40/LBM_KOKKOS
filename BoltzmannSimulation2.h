...
public:
	...
	{
		Kokkos::parallel_for("edwdewdw", Nx, KOKKOS_CLASS_LAMBDA(int x)
		{
			for (int y = 0; y < Ny; y++)
			{
	...
	void update_stream()
	{
		...
		Kokkos::parallel_for("aefef", Nx, KOKKOS_CLASS_LAMBDA(int x)
		{
			CREATE_DXS;
			CREATE_DYS;
	...
	void update_collide()
	{
		Kokkos::parallel_for("a100", Nx, KOKKOS_CLASS_LAMBDA(int x)
		{
			CREATE_INV;
			CREATE_DXS;
			CREATE_DYS;
